...
module.exports = (function () {
    ...
    function sortNotPlayedSongsDescendingByVotes() {
        ...
        player._list = lazy(playlist).initial(playlist.length -
                getIndexOfNextPlayingSong()).concat(getNotYetPlayedSongsStream().sortBy(function (song) {
            return song.votes;
        }, true)).toArray();
    }
    ...
    function add(song) {
        var songFromNotYetPlayedSongs = getNotYetPlayedSongsStream().find(function (song) {
            return song.id === song.id;
        });
        if (!songFromNotYetPlayedSongs) {
            song.votes = 1;
            player.add(song);
            if (songEndTime < Date.now()) {
                player.next();
            }
        } else {
